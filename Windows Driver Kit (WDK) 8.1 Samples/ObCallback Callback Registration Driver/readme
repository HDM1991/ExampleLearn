...
这两个功能同时只能使用其中一个，而且就使用的这个功能来讲，也只能针对一个进程，毕竟是一个 example。
...
然后是 ObjectPreCallback callback routine 和 ObjectPostCallback callback routine 被调用的上下文。文档中只是说 
This routine is called at PASSIVE_LEVEL in an arbitrary thread context with normal kernel APCs disabled。更确切一点应该是运行在执行该操作的进程的任意线程上下文中。