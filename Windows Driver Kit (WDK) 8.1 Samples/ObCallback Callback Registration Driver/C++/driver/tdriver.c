...
BOOLEAN TdProcessNotifyRoutineSet2 = FALSE; // Process notify routines 是否已经设置

// TODO(H): 变量命名，再次差评

// allow filter the requested access
BOOLEAN TdbProtectName = FALSE;             // 开启保护模式
BOOLEAN TdbRejectName = FALSE;              // 开启禁止模式
...
{
    NTSTATUS Status;
    UNICODE_STRING NtDeviceName = RTL_CONSTANT_STRING (TD_NT_DEVICE_NAME);  // TODO(H): RTL_CONSTANT_STRING 第一次见
    ...
    // TODO(H): DbgPrintEx 这个函数之前貌似也没有用过
    ...
    // TODO(H): ObGetFilterVersion 这个函数也查下，貌似只在 Vista 之后才加上的
