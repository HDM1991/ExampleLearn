...
BOOLEAN bCallbacksInstalled = FALSE; // 回调函数是否已设置


#define CB_PROCESS_TERMINATE 0x0001 // PROCESS_TERMINATE
#define CB_THREAD_TERMINATE  0x0001 // THREAD_TERMINATE


// TODO(H): 这全局变量的命名，差评

//  The following are for setting up callbacks for Process and Thread filtering
PVOID pCBRegistrationHandle = NULL; // 标识通过 ObRegisterCallbacks 注册的回调函数，用于之后对应的删除操作
...
TD_CALLBACK_REGISTRATION CBCallbackRegistration = {0};  // 传给回调函数的参数

// Here is the protected process
WCHAR   TdwProtectName[NAME_SIZE+1] = {0}; // 要保护或者禁止的进程的名字
PVOID   TdProtectedTargetProcess = NULL;   // 要保护的进程的 EPROCESS 结构
HANDLE  TdProtectedTargetProcessId = {0};  // 要保护的进程的 ID
...
)
{
    ...
    if (bCallbacksInstalled == FALSE) {
        ...
        CBObRegistration.Altitude                   = CBAltitude;   // TODO(H): 不太明白
...
)
{
    ...
    if (PreInfo->ObjectType == *PsProcessType)  {
        ...
        // TODO(H): ObjectPreCallback routine 所处的上下文是什么样的？MSDN 说它运行在任意线程的上下文。但我想说的是，它是运行在
        //          对应该操作的进程的任意线程上下文中吗？从这里来看，是这样的
        ...
        // TODO(H): PsGetThreadProcessId 这个函数在MSDN上查不到，但在WRK的源码中可以查到。功能就是
        //          获取ETHREAD线程对象的进程ID
